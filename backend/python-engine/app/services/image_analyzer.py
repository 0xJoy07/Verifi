...
class ImageContentExtractor:
    ...
    def __init__(self):
        ...
        print(f"[EXTRACTOR]   API key set:  {bool(self.api_key)}", flush=True)
    ...
    async def _extract_vision(self, image_bytes: bytes) -> str:
        """
        Send the image to the vision model via OpenRouter.
        Uses the correct MIME type based on image magic bytes.
        Retries once on 429 rate-limit with a short back-off.
        Falls back to a secondary free model if the primary is unavailable.
        """
        if not self.api_key:
            raise RuntimeError("OPENROUTER_API_KEY not set — cannot use vision model")

        # Detect MIME type from magic bytes instead of hardcoding jpeg
        ...
        # Primary model + fallback in case of rate limiting
        models_to_try = [self.vision_model]
        if "gemma-4-26b" in self.vision_model:
            models_to_try.append("google/gemma-4-31b-it:free")
        elif "gemma-4-31b" in self.vision_model:
        ...
        for model_id in models_to_try:
            ...
            try:
                async with httpx.AsyncClient(timeout=60.0) as client:
                    ...
                    if response.status_code == 429:
                        print(
                            f"[EXTRACTOR] WARNING: {model_id} rate-limited (429), "
                            f"{'trying fallback...' if len(models_to_try) > 1 else 'giving up'}",
                            flush=True,
                        )
            ...
            if choices:
                ...
                used_model = data.get("model", model_id)
                print(f"[EXTRACTOR] Vision response from {used_model} ({len(content)} chars)", flush=True)
                return content
